...
%% {Name, Pid, Ref}
-define(REDIS_GLOBAL_ETS, ?MODULE).
%% {Ref,Name}
-define(REDIS_GLOBAL_MON_ETS, redis_global_mon).
...
unregister_name(Name, Pid) when is_pid(Pid) ->
  RedisName = to_redis_name(Name),
  case erlang:node(Pid) of
    N when N =:= node() -> gen_server:call(?SERVER, {unregister_name, RedisName, Pid}, ?CALL_TIMEOUT);
    N2 -> gen_server:call({?SERVER, N2}, {unregister_name, RedisName, Pid}, ?CALL_TIMEOUT)
...
init([]) ->
  ...
  ets:new(?REDIS_GLOBAL_MON_ETS, [named_table, set, {read_concurrency, true}]),
  {ok, #state{}}.

handle_call({unregister_name, RedisName, Pid}, From, S) when is_binary(RedisName) ->
  ...
  case ets:lookup(?REDIS_GLOBAL_ETS, RedisName) of
    ...
    [{_, Pid, Ref}] ->
      ...
      ets:delete(?REDIS_GLOBAL_MON_ETS, Ref)
  end,
  proc_lib:spawn(fun() -> unregister_name_i(RedisName, Pid), gen_server:reply(From, ok) end),
  {noreply, S};
handle_call({register_name, RedisName, Pid}, _From, S) when is_binary(RedisName) ->
  ...
  case R of
    yes ->
      ...
      ets:insert(?REDIS_GLOBAL_MON_ETS, {Ref, RedisName});
    no ->
      pass
  end,
  {reply, R, S};
...
handle_info({'DOWN', Ref, _Type, _Object, _Info}, S) ->
  erlang:demonitor(Ref, [flush]),
  case ets:lookup(?REDIS_GLOBAL_MON_ETS, Ref) of
    [{_, Name}] ->
...
recv_shutdown(Acc, Total) when Acc >= Total -> ok;
recv_shutdown(Acc, Total) ->
  receive
    ok ->
      recv_shutdown(Acc + 1, Total)
  after 2000 ->
    recv_shutdown(Acc + 1, Total)
  end.