...
is_remsh_node(Node) when is_atom(Node) ->
  case catch rpc:call(Node, init, get_argument, [remsh], 1000) of
    {ok, _} -> true;
    _ -> false
  end.
...
handle_nodeup(Nodes, #state{node_list = NL, cluster_key = CK} = S) when is_list(Nodes) ->
